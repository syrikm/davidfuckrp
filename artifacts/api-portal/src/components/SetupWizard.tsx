...
interface SetupStatus {
  ...
  friendProxyConfigured?: boolean;
  integrationsReady?: boolean;
}

function buildSetupPrompt(
  apiKey: string,
  opts: { needStorage: boolean } = { needStorage: true }
): string {
  const steps: string[] = [];
  let stepNum = 0;
  ...
  if (opts.needStorage) {
   ...
   在 code_execution 工具中执行以下 JavaScript，为本项目开通独立的 GCS 存储桶，确保动态节点 / 已注册子节点信息在重新 publish 后不丢失：
  ...
  return `请帮我完成 Replit2Api 母代理的初始配置，严格按以下步骤执行：
...
export default function SetupWizard({ baseUrl, onComplete, onDismiss }: Props) {
  ...
  useEffect(() => {
    setTimeout(() => {
      setMessages([
        makeMsg(
          "agent",
          "你好！我是配置助手。\n\n这里是母代理 Portal。母代理本身只需要完成访问密码和云端存储初始化；真正的模型执行由后续接入的子节点负责。\n\n（我会自动检测已就绪的部分，只生成你真正需要的配置步骤）",
          {
            actions: [
  ...
  const runCheck = useCallback(async () => {
    ...
    const baseOk = status.configured;
    const nodeReady = status.friendProxyConfigured === true || status.integrationsReady === true;
    const nodeHint = nodeReady
      ? "✓ 已检测到至少一个执行节点 / 子节点"
      : "• 当前尚未检测到执行节点；稍后请去 Cluster 添加，或等待子节点自动注册";

    if (baseOk && status.storageReady) {
      addAgent(
        `配置成功！\n\n✓ 访问密码已设置\n✓ 云端持久化存储已开通\n${nodeHint}\n\n接下来你可以：\n1. 在 Tutorial 复制 /v1 Base URL 给客户端\n2. 在 Cluster 确认执行节点状态`,
        {
          actions: [
      ...
      addAgent(
        "访问密码已经就绪！\n\n还差最后一步：开通云端持久化存储（App Storage），确保动态节点 / 已注册子节点信息在重新 publish 后不丢失。\n\n请将下方指令复制发给 Replit Agent：",
        {
      ...
      addAgent(
        ...
        {
          copyBlocks: [{ text: buildSetupPrompt(chosenKey, { needStorage }) }],
          actions: [{ label: "已重启，重新检测", value: "check", primary: true }],
        },
        300
      );
  ...
  const handleKeySubmit = useCallback(async () => {
    ...
    addAgent(
      `好的，密码已记录！请将下方指令完整复制，发送给 Replit Agent。它会帮你完成母代理初始化（密码已写入指令，Agent 直接设置，无需你再输入）：${skippedNote}`,
      {
        copyBlocks: [{ text: buildSetupPrompt(key, { needStorage }) }],
